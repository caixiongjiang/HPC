...
TEST(test_homework, homework1_flatten1) {
  ...
  f1.Rand();
  f1.Show();
  LOG(INFO)<< "原来Tensor的shape为:" << f1.raw_shapes().size();
  f1.Flatten(true);
  f1.Show();
  LOG(INFO)<< "改变后Tensor的shape为:" << f1.raw_shapes().size();
...
TEST(test_homework, homework2_padding1) {
  ...
  ASSERT_EQ(tensor.raw_shapes().at(1), 7);
  ASSERT_EQ(tensor.raw_shapes().at(2), 12);
